...
// 1. Update allWeightData to have two series
const newDataString = `const allWeightData = {
  '7days': [
    { day: 'Mon', weight: 1120, target: 1180 }, { day: 'Tue', weight: 1150, target: 1180 }, { day: 'Wed', weight: 1140, target: 1190 },
    { day: 'Thu', weight: 1190, target: 1200 }, { day: 'Fri', weight: 1220, target: 1220 }, { day: 'Sat', weight: 1260, target: 1230 }, { day: 'Sun', weight: 1280, target: 1250 }
  ],
  '30days': [
    { day: 'Week 1', weight: 1100, target: 1120 }, { day: 'Week 2', weight: 1150, target: 1150 }, { day: 'Week 3', weight: 1210, target: 1180 }, { day: 'Week 4', weight: 1280, target: 1210 }
  ],
  'year': [
    { day: 'Jan', weight: 900, target: 1000 }, { day: 'Feb', weight: 950, target: 1020 }, { day: 'Mar', weight: 1000, target: 1040 }, { day: 'Apr', weight: 1050, target: 1080 },
    { day: 'May', weight: 1100, target: 1110 }, { day: 'Jun', weight: 1150, target: 1150 }, { day: 'Jul', weight: 1200, target: 1180 }, { day: 'Aug', weight: 1280, target: 1210 }
  ]
};`;

code = code.replace(/const allWeightData = \{[\s\S]*?\n  \]\n\};\n/, newDataString + '\n');

// 2. Replace the chart container and chart itself
const chartStart = `<div className="lg:col-span-2 bg-white p-6 rounded-lg transition-colors" style={{ boxShadow: "0px 3px 9px 0px \nrgba(162, 176, 190, 0.15)" }}>`;
// It might be split differently. Let's just use regex for the whole chart div.
const chartDivRegex = /<div className="lg:col-span-2[^>]*>([\s\S]*?)<\/ResponsiveContainer>\s*<\/div>\s*\{\/\* Quick Actions Panel \*\/\}/;

const newChartBlock = `<div className="lg:col-span-2 bg-[#18181b] p-6 rounded-2xl shadow-xl border border-gray-800 transition-colors">
...
code = code.replace(chartDivRegex, newChartBlock);
