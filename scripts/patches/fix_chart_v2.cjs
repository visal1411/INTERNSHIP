...
// 1. Update allWeightData to have two series, with target higher than weight
...
// 2. Replace the chart container and chart itself, AND the Quick Actions panel
const chartDivRegex = /<div className="lg:col-span-2[^>]*>([\s\S]*?)<\/ResponsiveContainer>\s*<\/div>\s*\{\/\* Quick Actions Panel \*\/\}\s*<div className="mt-6 pt-6 border-t border-gray-100 ">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/;
...
console.log("Chart patched with v2.");
